...
自动在指定目录下查找含 **WinExe** 输出的 `.csproj`（多个时取**修改时间最新**的一个），使用 `dotnet build` 以 **Debug** 模式编译（`-p:WarningLevel=0`），输出到 `build/<版本标签>/`，删除 `.pdb` 后可选地将输出目录打成压缩包。默认 **`-f 7z`**：若 PATH 中能找到 **`7z` / `7z.exe`** 则生成 **`.7z`**，否则打印提示并改用 **zip**。

## 环境要求

- **.NET SDK**（需能在命令行执行 `dotnet build`）
- **（推荐）7-Zip**：默认 **`-f 7z`**，需本机 PATH 中有 `7z` / `7z.exe` 才会产出 `.7z`；找不到时自动退回 zip。使用 **`-f zip`** 可不依赖 7-Zip。

## C# 项目需满足的条件

1. 存在标记为 **WinExe** 的 `.csproj`（多个时取**修改时间最新**的一个）。
2. 若 `.csproj` 中包含 **`<Version>`**，输出子目录名为版本相关标签；若还有 **`<FileVersion>`**，标签为 `Version-FileVersion`（`FileVersion` 中的半角空格会去掉）。缺少 `<Version>` 时，标签为 **csproj 文件名**（不含扩展名）。压缩包默认为 **`.7z`**（PATH 中有 `7z` 时），否则为 **`.zip`**；**`-f zip`** 时强制为 **`.zip`**。
...
```powershell
pack.exe              # 编译并打包为 7z（默认，无 7z 则 zip）
pack.exe -f zip       # 强制 zip
pack.exe -c           # 仅编译，不打包
```
...
- 打包文件（未使用 `-c` 时）：`build/<应用名>-V<版本标签>.7z`（默认且 PATH 中有 `7z` 时），否则为同名的 **`.zip`**；**`-f zip`** 时始终为 **`.zip`**。
...
- **Python 3.x**（脚本仅使用标准库，**pip 无需安装额外包**；默认 `-f 7z`，若本机无 `7z` 会自动退回 zip。）
...
| `-f zip`，`-f 7z`，`--format` | **7z**（默认，若 PATH 中有 `7z` 则用 7z，否则 zip 并打印提示）；**zip** 强制 zip。 |
...
```bash
python pack.py -d C:\path\to\project           # 指定目录，默认 7z
python pack.py -d C:\path\to\project -f zip    # 强制 zip
...
- **「7z 打包失败」**：`7z` 已找到但压缩命令失败时退出；若希望避免走 7z，改用 **`-f zip`**。**默认却得到 zip**：说明 PATH 中未找到 `7z`，脚本已自动退回 zip，可按提示安装 7-Zip 并配置 PATH。
